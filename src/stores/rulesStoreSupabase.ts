...
interface RulesState {
  ...
  addRule: (rule: Omit<Rule, 'id' | 'createdAt' | 'updatedAt'>, userId: string) => Promise<Rule>
  updateRule: (id: string, updates: Partial<Rule>, userId: string) => Promise<void>
  deleteRule: (id: string, userId: string) => Promise<void>
  toggleRuleStatus: (id: string, userId: string) => Promise<void>
  duplicateRule: (id: string, userId: string) => Promise<Rule>
  
  // Szablony
  createRuleFromTemplate: (templateId: string, userId: string) => Promise<Rule>
...
export const useRulesStore = create<RulesState>((set, get) => ({
  ...
  // Aktualizacja reguły
  updateRule: async (id, updates, userId) => {
    set({ loading: true, error: null })
    try {
  ...
  toggleRuleStatus: async (id, userId) => {
    const rule = get().rules.find(r => r.id === id)
    if (rule) {
      await get().updateRule(id, { status: rule.status === 'active' ? 'inactive' : 'active' }, userId)
    }
  },

  // Duplikowanie reguły
  duplicateRule: async (id, userId) => {
    const rule = get().rules.find(r => r.id === id)
    if (!rule) {
  ...
  createRuleFromTemplate: async (templateId, userId) => {
    set({ loading: true, error: null })
    try {
